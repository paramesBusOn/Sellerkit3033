...
class LevelofApi {
  static Future<LevelofModal> getData(
    sapUserId,
  ) async {
    int resCode = 500;
    try {
...
// Config config = Config();
      print(Url.queryApi + 'SkClientPortal/GetallMaster?MasterTypeId=20');
      print("token:"+ConstantValues.token);
      // await config.getSetup(); 
      final response = await http.get(Uri.parse(Url.queryApi + 'SkClientPortal/GetallMaster?MasterTypeId=20'),
          headers: {
      ...
      // print("level Type"+response.body.toString());
      ...
      //  return enquiryTypeModal;
        return LevelofModal.fromJson(
            json.decode(response.body), response.statusCode);
      } else {
        print("Error: ${json.decode(response.body)}");
        return LevelofModal.issues(json.decode(response.body), response.statusCode);
      }
    } catch (e) {
     print("Exception: " + e.toString());
      return LevelofModal.error(e.toString(), resCode);
    }
  }
