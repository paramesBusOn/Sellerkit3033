...
class GetLeadphoneApi {
  static Future<GetLeadphoneModal> getData(
  ) async {
    int resCode = 500;
    try {
// Config config = Config();
// print("Lead Status api::"+Url.queryApi +'SkClientPortal/GetallMaster?MasterTypeId=15');
      // await config.getSetup();
       final response = await http.get(Uri.parse(Url.queryApi +'SkClientPortal/GetallMaster?MasterTypeId=15'),
          headers: {
      ...
      // log("LEAD_STATUS_phone"+response.body.toString());
      ...
      //  islol.kill(priority: Isolate.immediate);
       return GetLeadphoneModal.fromJson(json.decode(response.body), resCode);
      } else {
       print("Error: ${json.decode(response.body)}");
        return GetLeadphoneModal.error('Error', resCode);
      }
    } catch (e) {
     print("Exception: " + e.toString());
      return GetLeadphoneModal.error(e.toString(), resCode);
    }
  }
