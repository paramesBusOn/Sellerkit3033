...
class UpdatevisitApi {
  static Future<updatevisitmodel> postvisit(
    ...
    try {
      ...
      final response = await http.post(
          Uri.parse(Url.queryApi + 'Sellerkit_Flexi/v2/UpdateVisitplan'),
          headers: {
  ...
  "att4": null
      ...
      if (response.statusCode == 200) {
        return updatevisitmodel.fromJson(
            json.decode(response.body), response.statusCode);
      } else {
        //  print("Error: ${json.decode(response.body)}");
        return updatevisitmodel.issue(json.decode(response.body),  response.statusCode);
      }
    } catch (e) {
      print("Exception: $e");
      return updatevisitmodel.exception(e.toString(), resCode);
    }
  }
}
